...
# ================================================================
# ตระกูลโจทย์ของชั้นประถม (ป.5 · ป.6)
# ----------------------------------------------------------------
# กฎเหล่านี้อยู่ท้ายไฟล์โดยตั้งใจ — กฎด้านบนแคบกว่าและต้องได้จับก่อน
# ทุกกฎคิดคำตอบใหม่จากตัวเลขในโจทย์ ไม่ได้อ่านค่าจากตัวสร้างข้อสอบ
# ================================================================

@rule(r"^อัตราส่วน (\d+) : (\d+) เท่ากับอัตราส่วน (\d+) : ใด$")
def _(q, m):
    a, b, c = int(m[1]), int(m[2]), int(m[3])
    if c % a:
        raise NotPlainData("อัตราส่วนที่ให้มาไม่ได้เป็นพหุคูณลงตัว")
    # ไล่หา d ที่ทำให้ a·d = b·c จริง แทนการคูณด้วยตัวคูณที่มองเห็น
    hits = [d for d in range(1, b * c + 1) if a * d == b * c]
    if len(hits) != 1:
        raise NotPlainData("หา d ไม่ได้คำตอบเดียว")
    want(q, hits[0], "คูณไขว้แล้วไล่หาค่าที่สอดคล้อง")


@rule(r"^วงกลมรัศมี ([\d.]+) เซนติเมตร มีความยาวรอบรูปกี่เซนติเมตร")
def _(q, m):
    r = Fraction(m[1])
    # ความยาวรอบรูป = เส้นผ่านศูนย์กลาง × พาย — ใช้เส้นผ่านศูนย์กลาง ไม่ใช่ 2πr
    want(q, (2 * r) * Fraction(22, 7), "เส้นผ่านศูนย์กลางคูณพาย")


@rule(r"^วงกลมรัศมี ([\d.]+) เซนติเมตร มีพื้นที่กี่ตารางเซนติเมตร")
def _(q, m):
    r = Fraction(m[1])
    # พื้นที่ = ครึ่งหนึ่งของ (ความยาวรอบรูป × รัศมี) — ที่มาของสูตร πr² ในหนังสือเรียน
    want(q, (2 * r * Fraction(22, 7)) * r / 2, "ครึ่งของรอบรูปคูณรัศมี")


@rule(r"^ปริซึม (\d+) เหลี่ยมมี(หน้า|ขอบ)ทั้งหมดกี่")
def _(q, m):
    n = int(m[1])
    # นับจากการประกอบจริง: หน้าข้าง n หน้า บวกหน้าตัดสองหน้า · ขอบ = ขอบหน้าตัดสองวง + ขอบข้าง
    faces = n + 2
    edges = len({("bottom", i) for i in range(n)} | {("top", i) for i in range(n)}
                | {("side", i) for i in range(n)})
    want(q, faces if m[2] == "หน้า" else edges, "ประกอบปริซึมแล้วนับ")


@rule(r"^พีระมิดฐาน (\d+) เหลี่ยมมีหน้าทั้งหมดกี่หน้า$")
def _(q, m):
    n = int(m[1])
    want(q, n + 1, "หน้าข้าง n หน้า บวกฐานหนึ่งหน้า")


@rule(r"^สินค้าราคา (\d+) บาท ลดราคาร้อยละ (\d+) ต้องจ่ายเงินกี่บาท$")
def _(q, m):
    price, pct = int(m[1]), int(m[2])
    want(q, price - Fraction(price * pct, 100), "ราคาเต็มลบส่วนลด")


@rule(r"^ระฆังใบแรกตีทุก (\d+) นาที ใบที่สองตีทุก (\d+) นาที")
def _(q, m):
    a, b = int(m[1]), int(m[2])
    # ไล่ทีละนาทีจนเจอนาทีแรกที่ทั้งสองใบตีพร้อมกัน ไม่ใช้สูตร ค.ร.น.
    hits = [t for t in range(1, a * b + 1) if t % a == 0 and t % b == 0]
    want(q, hits[0], "ไล่เวลาทีละนาทีจนตีพร้อมกัน")


@rule(r"^ริบบิ้นสองเส้นยาว (\d+) เซนติเมตรและ (\d+) เซนติเมตร")
def _(q, m):
    a, b = int(m[1]), int(m[2])
    want(q, max(d for d in range(1, min(a, b) + 1) if a % d == 0 and b % d == 0),
         "ไล่ความยาวท่อนที่หารทั้งสองเส้นลงตัว")


@rule(r"^รูปสามเหลี่ยมมีมุมสองมุมขนาด (\d+) องศาและ (\d+) องศา")
def _(q, m):
    want(q, 180 - int(m[1]) - int(m[2]), "มุมภายในรวม 180 องศา")


@rule(r"^รูปสามเหลี่ยมหน้าจั่วมีมุมยอด (\d+) องศา")
def _(q, m):
    apex = int(m[1])
    # มุมฐานสองมุมเท่ากัน — ไล่หาค่าที่ทำให้ผลรวมเป็น 180 พอดี
    hits = [b for b in range(1, 3600) if apex * 2 + b * 2 == 360]
    if len(hits) != 1:
        raise NotPlainData("หามุมฐานไม่ได้คำตอบเดียว")
    want(q, Fraction(hits[0], 2), "ไล่หามุมฐานที่ทำให้ผลรวมเป็น 180")


@rule(r"^ซื้อของราคาชิ้นละ ([\d.]+) บาท จำนวน (\d+) ชิ้น จ่ายเงินไป ([\d.]+) บาท")
def _(q, m):
    price, qty, paid = Fraction(m[1]), int(m[2]), Fraction(m[3])
    total = sum([price] * qty, Fraction(0))       # บวกซ้ำ ไม่ใช้การคูณ
    want(q, paid - total, "บวกราคาทีละชิ้นแล้วหักจากเงินที่จ่าย")


@rule(r"^ซื้อสมุดราคาเล่มละ ([\d.]+) บาท จำนวน (\d+) เล่ม และซื้อปากกาอีก ([\d.]+) บาท")
def _(q, m):
    price, qty, extra = Fraction(m[1]), int(m[2]), Fraction(m[3])
    want(q, sum([price] * qty, Fraction(0)) + extra, "บวกราคาทีละเล่มแล้วบวกปากกา")


@rule(r"^เชือกยาว (\d+) เมตร (\d+) เซนติเมตร")
def _(q, m):
    want(q, Fraction(int(m[1]) * 100 + int(m[2]), 100), "รวมเป็นเซนติเมตรแล้วหารกลับ")


@rule(r"^ข้าวสารหนัก (\d+) กิโลกรัม (\d+) กรัม")
def _(q, m):
    want(q, Fraction(int(m[1]) * 1000 + int(m[2]), 1000), "รวมเป็นกรัมแล้วหารกลับ")


@rule(r"^สินค้าราคา (\d+) บาท ลดราคาร้อยละ (\d+) แล้วลดจากราคาใหม่อีกร้อยละ (\d+)")
def _(q, m):
    price, p1, p2 = int(m[1]), int(m[2]), int(m[3])
    step = price - Fraction(price * p1, 100)
    want(q, step - step * Fraction(p2, 100), "หักส่วนลดทีละรอบจากราคาที่เหลือ")


@rule(r"^มีลูกอม (\d+) เม็ด แบ่งให้น้อง \((\d+)\)/\((\d+)\) ของทั้งหมด "
      r"แล้วแบ่งส่วนที่เหลืออีก \((\d+)\)/\((\d+)\)")
def _(q, m):
    total, a, b, c, d = (int(m[i]) for i in range(1, 6))
    left = total - Fraction(total * a, b)
    want(q, left - left * Fraction(c, d), "หักทีละรอบจากส่วนที่เหลือจริง")


@rule(r"^มีข้าวสาร (\d+) กิโลกรัม ขายไป \((\d+)\)/\((\d+)\) ของทั้งหมด "
      r"แล้วขายส่วนที่เหลืออีก \((\d+)\)/\((\d+)\)")
def _(q, m):
    total, a, b, c, d = (int(m[i]) for i in range(1, 6))
    left = total - Fraction(total * a, b)
    want(q, left - left * Fraction(c, d), "หักทีละรอบจากส่วนที่เหลือจริง")
...
@rule(r"^ภาชนะทรงสี่เหลี่ยมมุมฉากกว้าง (\d+) เซนติเมตร ยาว (\d+) เซนติเมตร "
      r"สูง (\d+) เซนติเมตร จุน้ำ")
def _(q, m):
    w, d, h = int(m[1]), int(m[2]), int(m[3])
    want(q, Fraction(w * d * h, 1000), "ปริมาตรเป็นลูกบาศก์เซนติเมตรแล้วเปลี่ยนเป็นลิตร")


# ต้องปิดท้ายด้วยคำถามเรื่องปริมาตร ไม่งั้นกฎนี้ไปกินโจทย์ความหนาแน่นของวิทย์ ม.1
# ซึ่งขึ้นต้นเหมือนกันทุกคำ แต่ถามหามวลต่อปริมาตร
@rule(r"^กล่องทรงสี่เหลี่ยมมุมฉากกว้าง (\d+) เซนติเมตร ยาว (\d+) เซนติเมตร "
      r"สูง (\d+) เซนติเมตร มีปริมาตรกี่ลูกบาศก์เซนติเมตร$")
def _(q, m):
    w, d, h = int(m[1]), int(m[2]), int(m[3])
    # นับลูกบาศก์หน่วยทีละชั้น แทนการคูณสามจำนวนรวดเดียว
    want(q, sum(w * d for _ in range(h)), "นับลูกบาศก์หน่วยทีละชั้น")


@rule(r"^รูปสี่เหลี่ยมด้านขนานมีฐานยาว (\d+) เซนติเมตร และสูง (\d+) เซนติเมตร")
def _(q, m):
    base, height = int(m[1]), int(m[2])
    want(q, sum([base] * height), "ตัดเป็นรูปสี่เหลี่ยมมุมฉากแล้วนับทีละแถว")


@rule(r"^รูปสี่เหลี่ยมด้านขนานมีด้านประชิดยาว (\d+) เซนติเมตร และ (\d+) เซนติเมตร")
def _(q, m):
    a, b = int(m[1]), int(m[2])
    want(q, a + b + a + b, "บวกความยาวทั้งสี่ด้าน")


@rule(r"^รูปตัวแอลได้จากรูปสี่เหลี่ยมผืนผ้ากว้าง (\d+) เซนติเมตร สูง (\d+) เซนติเมตร "
      r"แล้วตัดมุมขวาบนออกเป็นสี่เหลี่ยมผืนผ้ากว้าง (\d+) เซนติเมตร สูง (\d+) เซนติเมตร")
def _(q, m):
    a, b, c, d = (int(m[i]) for i in range(1, 5))
    if c >= a or d >= b:
        raise NotPlainData("ส่วนที่ตัดออกใหญ่กว่ารูปเดิม")
    # แบ่งเป็นสองผืนแล้วบวก แทนการเอาพื้นที่ใหญ่ลบส่วนที่ตัด
    want(q, (a - c) * b + c * (b - d), "แบ่งรูปตัวแอลเป็นสี่เหลี่ยมสองผืนแล้วบวก")


@rule(r"^เงินจำนวนหนึ่งถูกใช้ไป \((\d+)\)/\((\d+)\) ของทั้งหมด "
      r"แล้วส่วนที่เหลือมากกว่าส่วนที่ใช้ไป (\d+) บาท")
def _(q, m):
    a, b, diff = int(m[1]), int(m[2]), int(m[3])
    gap = 1 - 2 * Fraction(a, b)
    if gap <= 0:
        raise NotPlainData("ส่วนที่เหลือไม่มากกว่าส่วนที่ใช้")
    total = Fraction(diff) / gap
    # ยืนยันย้อนกลับ: ใส่ค่าที่ได้กลับเข้าเงื่อนไขต้องได้ผลต่างเท่าเดิม
    if (total - total * Fraction(a, b)) - total * Fraction(a, b) != diff:
        raise NotPlainData("แทนค่ากลับแล้วไม่ตรงเงื่อนไข")
    want(q, total, "ตั้งส่วนต่างเป็นเศษส่วนของทั้งหมดแล้วย้อนกลับ")


@rule(r"^ตารางถนนกว้าง (\d+) ช่อง สูง (\d+) ช่อง")
def _(q, m):
    w, h = int(m[1]), int(m[2])
    # ตารางสะสม — คนละวิธีกับสูตรการจัดหมู่
    g = [[1] * (w + 1) for _ in range(h + 1)]
    for i in range(1, h + 1):
        for j in range(1, w + 1):
            g[i][j] = g[i - 1][j] + g[i][j - 1]
    want(q, g[h][w], "สะสมจำนวนเส้นทางทีละช่อง")


@rule(r"^ข้อมูล (\d+) จำนวนมีค่าเฉลี่ยเท่ากับ (\d+) ถ้าเพิ่มข้อมูลอีกหนึ่งจำนวน"
      r"ซึ่งมีค่า (\d+)")
def _(q, m):
    n, mean, extra = int(m[1]), int(m[2]), int(m[3])
    want(q, Fraction(n * mean + extra, n + 1), "คืนผลรวมเดิมแล้วเฉลี่ยใหม่")


@rule(r"^แบ่งเงิน (\d+) บาทตามอัตราส่วน (\d+) : (\d+) คนแรกได้เงินกี่บาท$")
def _(q, m):
    total, a, b = int(m[1]), int(m[2]), int(m[3])
    unit = Fraction(total, a + b)
    want(q, sum([unit] * a, Fraction(0)), "หาค่าหนึ่งส่วนแล้วบวกทีละส่วน")


@rule(r"^แบบรูปของจำนวนเริ่มต้นด้วย (\d+) แล้วเพิ่มขึ้นครั้งละ (\d+) พจน์ที่ (\d+)")
def _(q, m):
    a0, d, n = int(m[1]), int(m[2]), int(m[3])
    v = a0
    for _ in range(n - 1):
        v += d
    want(q, v, "ไล่บวกทีละพจน์")


@rule(r"^แบบรูปของจำนวนเริ่มต้นด้วย (\d+) แล้วเพิ่มขึ้นครั้งละ (\d+) "
      r"ผลบวกของ (\d+) พจน์แรก")
def _(q, m):
    a0, d, n = int(m[1]), int(m[2]), int(m[3])
    want(q, n * (2 * a0 + (n - 1) * d) // 2, "สูตรผลบวกลำดับเลขคณิต")


@rule(r"ก้อนล่างกว้าง (\d+) ยาว (\d+) สูง (\d+) เซนติเมตร และก้อนบนกว้าง (\d+) "
      r"ยาว (\d+) สูง (\d+) เซนติเมตร")
def _(q, m):
    a, b, c, d, e, f = (int(m[i]) for i in range(1, 7))
    want(q, sum(a * b for _ in range(c)) + sum(d * e for _ in range(f)),
         "นับลูกบาศก์หน่วยทีละชั้นของทั้งสองก้อน")


@rule(r"^จำนวนนับ n ที่ไม่เกิน (\d+) ซึ่งหารด้วย (\d+) เหลือเศษ (\d+) "
      r"และหารด้วย (\d+) เหลือเศษ (\d+)")
def _(q, m):
    hi, m1, r1, m2, r2 = (int(m[i]) for i in range(1, 6))
    want(q, sum(1 for n in range(1, hi + 1) if n % m1 == r1 and n % m2 == r2),
         "ไล่ตรวจทีละจำนวน")


@rule(r"^สินค้าราคาทุน (\d+) บาท ติดป้ายราคาเพิ่มจากทุนร้อยละ (\d+) "
      r"แล้วลดราคาจากป้ายร้อยละ (\d+)")
def _(q, m):
    cost, up, down = int(m[1]), int(m[2]), int(m[3])
    label = cost + Fraction(cost * up, 100)
    sell = label - label * Fraction(down, 100)
    want(q, sell - cost, "คิดราคาป้ายแล้วหักส่วนลด จึงเทียบกับทุน")


@rule(r"^คนแรกทำงานชิ้นหนึ่งเสร็จใน (\d+) วัน คนที่สองทำงานชิ้นเดียวกันเสร็จใน (\d+) วัน")
def _(q, m):
    a, b = int(m[1]), int(m[2])
    rate = Fraction(1, a) + Fraction(1, b)
    days = 1 / rate
    if rate * days != 1:
        raise NotPlainData("อัตรารวมคูณเวลาแล้วไม่ได้งานหนึ่งชิ้น")
    want(q, days, "รวมอัตราการทำงานต่อวันแล้วกลับเศษส่วน")
