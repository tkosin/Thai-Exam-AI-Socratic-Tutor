...
@rule(r"นักเรียนกลุ่มหนึ่ง (\d+) คน สอบได้คะแนนเฉลี่ย (\d+) คะแนน ถ้าแยกเป็นชาย (\d+) "
      r"คน คะแนนเฉลี่ย (\d+) คะแนน จงหาคะแนนเฉลี่ยของนักเรียนหญิง")
def _(q, m):
...
@rule(r"^จงหาจำนวนเต็มบวก n ที่น้อยที่สุดที่ทำให้ (\d+)\^n (?:&gt;|>) (\d+)")
def _(q, m):
    a, b = int(m[1]), int(m[2])
