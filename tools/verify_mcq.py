...
ข้อ 1-4 **บังคับทุกวิชา** — ข้อ 1-3 เป็นข้อพังจริง ส่วนข้อ 4 ทั้งคลังผ่านแล้วหลังรอบ
สลับตำแหน่งด้วย tools/rebalance_choices.py จึงบังคับได้เลยเพื่อกันถอยหลัง

ข้อ 5 **บังคับเฉพาะวิชาที่เปิด `mcq_gate: true`** ใน questions/topics.json — แก้ความยาว
ตัวเลือกต้องเขียนตัวลวงใหม่ ไม่ใช่สลับตำแหน่ง จึงเป็นงานเขียนเนื้อหาราว 450 ข้อ
วิชาใหม่เปิดตั้งแต่วันแรก ส่วนวิชาเดิมรายงานตัวเลขไว้ให้เห็นว่าเป็นหนี้ค้าง
...
def check_course(course, qs, gated):
    ...
    # ตำแหน่งเฉลยบังคับทุกวิชา ไม่ต้องรอ mcq_gate — ทั้งคลังผ่านแล้วหลังรอบสลับตำแหน่ง
    # (tools/rebalance_choices.py) ด่านนี้จึงมีไว้กันถอยหลัง ไม่ใช่หนี้ที่ยังค้าง
    for k in "กขคง":
        share = pos[k] / n
        if share > POS_MAX:
            errs.append(f"{where}: เฉลยอยู่ที่ '{k}' ถึง {share * 100:.0f}% "
                        f"({pos[k]}/{n}) — เดาตัวเดียวรวดก็ได้คะแนนเท่านี้ "
                        "· แก้ด้วย tools/rebalance_choices.py")
        elif share < POS_MIN:
            errs.append(f"{where}: เฉลยอยู่ที่ '{k}' แค่ {share * 100:.0f}% "
                        f"({pos[k]}/{n}) — ผู้เรียนตัดตัวเลือกนี้ทิ้งได้โดยไม่ต้องอ่าน "
                        "· แก้ด้วย tools/rebalance_choices.py")

    # ความยาวตัวเลือกยังเป็นหนี้ค้าง — แก้ต้องเขียนตัวลวงใหม่ ไม่ใช่สลับตำแหน่ง
    # จึงยังบังคับเฉพาะวิชาที่เปิด mcq_gate (วิชาใหม่เปิดตั้งแต่วันแรก)
    if gated:
        if rate > LONGEST_MAX:
...
def selftest():
    ...
    # เฉลยยาวกว่าตัวลวงทุกข้อ แต่ตำแหน่งกระจายดีและตัวลวงไม่ซ้ำกัน
    # จึงแยกได้ว่าที่ฟ้องมาจากด่านความยาวจริง ไม่ใช่ด่านอื่นปนมา
    leaky = [Q("กขคง"[i % 4],
               [("ยาวมากกกกกกกกกกกกกก" if j == i % 4 else f"สั้น{j}")
    ...
    cases = [
        ...
        ("ตัวเลือกหนึ่งแทบไม่เคยเป็นเฉลย",
         ...
         ("เฉลยอยู่ที่ 'ง' แค่ 0%",)),
        ("เฉลยเป็นตัวที่ยาวที่สุดเกือบทุกข้อ", leaky, True,
         ("เฉลยเป็นตัวเลือกที่ยาวที่สุด 100%",)),
        ("ตำแหน่งเฉลยบังคับแม้วิชายังไม่เปิด mcq_gate",
         [Q("ก", even, s=f"โจทย์ที่ {i}") for i in range(24)], False,
         ("เฉลยอยู่ที่ 'ก' ถึง 100%",)),
        ("ความยาวตัวเลือกยังไม่บังคับถ้าวิชายังไม่เปิด mcq_gate", leaky, False, ()),
